...
type NewProductData = Omit<Product, 'id' | 'rating' | 'reviews' | 'createdAt'>;
type UpdateProductData = Omit<Product, 'id' | 'createdAt'>;
...
export async function createProduct(productData: NewProductData): Promise<Product> {
    ...
    // Construct the final product object to return
    // Since serverTimestamp is not resolved on the client, we omit createdAt
